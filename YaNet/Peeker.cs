...
namespace YaNet
{
    public class Peeker
    {
        ...
        public (Mark, Mark) ToPair(char delimiter)
        {
            int iL = IndentLevel("\t");

            var marks = Split(delimiter);

            return ((marks[0].Start + iL, marks[0].End), marks[1]);
        }

        public Mark ToItemScalar()
        {
            int indent = IndentLevel("\t");

            // if item has "- " feature

            return new Mark(_start + indent + 2, _end);
        }
        ...
        public Mark ToKey()
        {
            ...
            return new Mark(_start + indent, _end - 1);
        }

        public (Mark, Mark) ToPair(string delimiter)
        {
            int iL = IndentLevel("\t");
        ...
        {
            int countDelimiters = Counter(delimiter) + 1;

            Mark[] marks = new Mark[countDelimiters];
        ...
        public Mark[] Split(string delimiter)
        {
            int countDelimiters = Counter(delimiter) + 1;
            ...
            int slice = lenDelimiter + 1;

            for (start = _start, end = _start; end <= _end; end++)
            {
                bool isDelimiter = true;

                for (int j = 0; j < lenDelimiter; j++, end++)
                {
                    if (_buffer[end] != delimiter[j])
                    {
                        isDelimiter = false;
                        break;
                    }
                ...
                if (isDelimiter)
                {
                    marks[i++] = new Mark(start, end - slice);
                    start = end;
                }
            }
        ...
        public int IndentLevel(string indent)
        {
